...
SchoolClass _cls(String id, String name,
    {List<RoomSlot> slots = const [], List<String> students = const []}) {
  return SchoolClass(
    id: id,
    name: name,
    studentIds: students,
...
RoomSlot _slot(String slotId, String stanzaId,
    {int giorno = 6, String inizio = '15:00', String fine = '16:30'}) {
  return RoomSlot(
    slotId: slotId,
    stanzaId: stanzaId,
...
void main() {
  group('SlotConflictService.findConflicts', () {
    test('nessun conflitto se aule/giorni differenti', () {
      final classB = _cls('b', 'Cresima A', slots: [
        _slot('s1', 'stanza_1', giorno: 1),
      ]);
    ...
    test('conflitto interno: la classe ha già un impegno nello stesso orario', () {
      final classB = _cls('b', 'Cresima A', slots: [
        _slot('s1', 'Stanza_1', giorno: 1),
      ]);
      final conflicts = SlotConflictService.findConflicts(
...
test('conflitto tra classi: stessa aula, stesso giorno, orario sovrapposto',
        () {
      final classA = _cls('a', 'Cresima A');
      final classB = _cls('b', 'Comunione B', slots: [
        _slot('s1', 'Stanza_1', giorno: 1),
      ]);
      final conflicts = SlotConflictService.findConflicts(
        target: classA,
    ...
    test('warning capienza se la classe supera la capienza dell\'aula', () {
      final classA = _cls('a', 'Classe Numerosa',
  ...
  test('RoomSlot.overlaps rileva sovrapposizioni di orario', () {
    expect(
      _slot('a', 's', inizio: '15:00', fine: '16:00')
          .overlaps(_slot('b', 's', inizio: '15:30', fine: '16:30')),
      isTrue,
    );
    expect(
      _slot('a', 's', giorno: 1, inizio: '15:00', fine: '16:00')
