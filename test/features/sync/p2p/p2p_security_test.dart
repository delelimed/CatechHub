...
void main() {
  group('P2PSecurityService.computePairingCode', () {
    ...
    test(
      'chiavi efimere ordinate in modo canonico producono lo stesso codice',
      () {
        const sharedSecret = 'dGhpcyBpcyBhIHRlc3Qgc2VjcmV0';
    ...
    test(
      'senza sessionNonce le chiavi efimere bastano a vincolare il codice',
      () {
        const sharedSecret = 'dGhpcyBpcyBhIHRlc3Qgc2VjcmV0';
  ...
  group('P2PSecurityService.publicKeyMatchesAssociation', () {
    ...
    test('restituisce false se la chiave non corrisponde', () {
      ...
      expect(
        P2PSecurityService.publicKeyMatchesAssociation(assoc, 'different_key'),
        isFalse,
      );
    });
  });
  ...
  group('P2PSecurityService.computeForwardSecretKey (forward secrecy)', () {
    ...
    test(
      'il segreto cambia se l\'efimera remota cambia (chiave per-sessione)',
      () async {
        final x25519 = X25519();
