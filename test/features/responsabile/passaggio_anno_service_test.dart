...
Student _student(String id, String name,
    {String? classId, String stato = 'ATTIVO', String anno = '2026-2027'}) {
  return Student(
    id: id,
    name: name,
    surname: name,
...
void main() {
  ...
  group('PassaggioAnnoService.passaAnno', () {
    test('promuove la classe e aggiorna l\'anno di iscrizione degli attivi',
        () async {
      final classesRepo = ClassesRepository();
      final studentsRepo = StudentsRepository();

      await classesRepo.addClass(SchoolClass(
        id: 'class_pa_1',
      ...
      for (final id in ['s1', 's2', 's3']) {
        await studentsRepo.addStudent(_student(id, 'Nome',
            classId: 'class_pa_1'));
      }

      await LocalDatabase.parishConfig()
          .put(ParishConfig.storageKey, ParishConfig.empty.copyWith(
      ...
      final updated = classesRepo.getClassesSync().firstWhere((c) => c.id == 'class_pa_1');
    ...
    test('gli studenti RITIRATO vengono archiviati via removeFromClass',
        () async {
      final studentsRepo = StudentsRepository();
      await studentsRepo.addStudent(_student('s_rit', 'Marco',
          classId: 'class_pa_2', stato: 'RITIRATO'));
      ...
      final ritirato = studentsRepo.getAllStudentsSync()
          .firstWhere((s) => s.id == 's_rit');
