...
import '../responsabile/consensi_service.dart';
...
/// Card "Consensi & contributi" per l'account normale (catechista): mostra
/// lo stato della scheda di iscrizione firmata (consenso GDPR) e del
/// contributo volontario, con le azioni di registrazione firma/revoca e
/// aggiornamento del contributo. Mantiene uno stato locale per riflettere
/// subito le modifiche.
class _ConsensoContributoCard extends ConsumerStatefulWidget {
  final Student student;
  final bool isDark;
  final ColorScheme colorScheme;

  const _ConsensoContributoCard({
    required this.student,
    required this.isDark,
    required this.colorScheme,
  });
  ...
  @override
  ConsumerState<_ConsensoContributoCard> createState() =>
...
class _ConsensoContributoCardState
  ...
  Future<void> _revoca() async {
    final conferma = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
          'Revocare la firma della scheda di ${_student.name} ${_student.surname}? '
  ...
  Future<void> _aggiornaContributo() async {
    final ctrl = TextEditingController(
      text: _student.contributoVersato
          ? _student.contributoEuros.toStringAsFixed(2)
          : '20.00',
    );
    var versato = _student.contributoVersato;
    String? anno = _student.annoContributo;
  ...
  @override
  Widget build(BuildContext context) {
    final info = ConsensiService.info(_student);
    ...
    return _InfoCard(
      ...
      color: isDark ? colorScheme.primary : const Color(0xFF00838F),
      children: [
        Row(
          children: [
            Container(
            ...
            Expanded(
              child: Text(
                info.eFirmato && info.firma != null
                    ? 'Firmata il ${DateFormat('dd/MM/yyyy').format(info.firma!.toLocal())}'
                    : 'Nessuna scheda firmata',
                style: TextStyle(
                  fontSize: 12.5,
        ...
        if (info.scadenza != null) ...[
          ...
          Text(
            'Scadenza trattamento: ${DateFormat('dd/MM/yyyy').format(info.scadenza!.toLocal())}',
            style: TextStyle(
              fontSize: 12.5,
              color: isDark ? Colors.grey.shade400 : Colors.black54,
            ),
          ...
          children: [
            if (info.eFirmato)
              OutlinedButton.icon(
                ...
                label: const Text('Revoca firma'),
                style: OutlinedButton.styleFrom(foregroundColor: Colors.red),
              )
            else
              FilledButton.tonalIcon(
                onPressed: _registraScheda,
                icon: const Icon(Icons.task_alt_rounded, size: 18),
                label: const Text('Registra firma'),
              ),
            OutlinedButton.icon(
              onPressed: _aggiornaContributo,
              icon: const Icon(Icons.payments_rounded, size: 18),
              label: const Text('Contributo'),
            ),
          ],
        ),
      ],
