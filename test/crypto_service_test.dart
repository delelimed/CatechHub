...
void main() {
  group('EncryptionService - Cifratura/Decifratura', () {
    test('dovrebbe cifrare e decifrare con la stessa chiave', () async {
    ...
    test('dovrebbe fallire la decifratura con chiave errata', () async {
    ...
    test(
      'dovrebbe produrre output diverso per lo stesso input (nonce casuale)',
      () async {
        final testData = {'nome': 'Mario'};
        ...
        final dec1 = await EncryptionService.decryptData(encrypted1, password);
        final dec2 = await EncryptionService.decryptData(encrypted2, password);
        expect(dec1['nome'], equals('Mario'));
        expect(dec2['nome'], equals('Mario'));
      },
    );

    test('dovrebbe verificare la password corretta', () async {
      ...
      expect(
        await EncryptionService.verifyPassword(encrypted, password),
        isTrue,
      );
      expect(
        await EncryptionService.verifyPassword(encrypted, 'sbagliata'),
        isFalse,
      );
    });
  });
  ...
  group('EncryptionService - Sincronizzazione Multi-Dispositivo', () {
    test('dovrebbe gestire chiavi diverse per dispositivi diversi', () async {
      ...
      final payloadA = await EncryptionService.encryptData(
        datiInviatiDaA,
        chiaveCatechistaA,
      ...
      final datiRicevutiDaB = await EncryptionService.decryptData(
        payloadA,
        chiaveCatechistaA,
      );
    ...
    test(
      'dovrebbe gestire sincronizzazione sequenziale con due dispositivi',
      () async {
        ...
        final payloadPerA = await EncryptionService.encryptData(
          recordPerA,
          chiaveA,
          iterations: EncryptionService.fastShareIterations,
        );
        ...
        final payloadPerB = await EncryptionService.encryptData(
          recordPerB,
          chiaveB,
          iterations: EncryptionService.fastShareIterations,
        );
        ...
        final decifratoA = await EncryptionService.decryptData(
          payloadPerA,
          chiaveA,
        );
        final decifratoB = await EncryptionService.decryptData(
          payloadPerB,
          chiaveB,
        );
  ...
  group('EncryptionService - Derivazione Chiave', () {
    test('PBKDF2 dovrebbe generare chiavi deterministiche', () async {
    ...
    test('PBKDF2 dovrebbe generari chiavi diverse con salt diversi', () async {
      final password = 'test_password';
    ...
    test('la chiave derivata dovrebbe essere di 32 byte (AES-256)', () async {
      final password = 'test_password';
      final salt = EncryptionService.secureRandomBytes(16);
