...
- **Sync P2P Nearby** — Sincronizzazione continua in background via Bluetooth/WiFi Direct con crittografia end-to-end (X25519 ECDH + HKDF + AES-256-GCM), pairing code a 6 cifre anti-MitM, key pinning per dispositivo, forward secrecy e sync differenziale. **Scope per classe**: decidi quali classi condividere con ciascun dispositivo. **Doppio consenso**: se entrambi i dispositivi sono "Altro Catechista", la sincronizzazione richiede l'autorizzazione esplicita di entrambi prima dello scambio dati. Nella modalità Responsabile, la **catena di fiducia** fa sì che solo i dispositivi approvati dal Responsabile possano sincronizzarsi.
- **Modalità Responsabile Catechistico** — Area dedicata al responsabile della parrocchia: dashboard parrocchiale con riepilogo (classi attive, ragazzi iscritti, catechisti) e funzioni rapide, gestione delle classi raggruppate per percorso (attive/archiviate), iscrizioni con promozione e passaggio di anno automatico, consensi privacy con durata di validità, **Registro Trattamenti GDPR** (audit log firmato HMAC di ogni operazione), gestione catechisti, **logistica delle aule** (slot settimanali, controllo conflitti, occupazione tabellare), import ragazzi, rete parrocchiale e dispositivi fidati. Le classi condivise restano sincronizzate con i catechisti associati; la logistica (aule e orari) si propaga a tutti i dispositivi approvati.
- **Logistica visibile anche agli associati** — Quando la modalità Responsabile è attiva, i dispositivi "Associato" possono consultare in **sola lettura** le aule, le stanze e la tabella orario settimanale (occupazione delle stanze). La modifica delle aule resta riservata al Responsabile.
- **Catena di fiducia** — Nella modalità Responsabile ogni dispositivo collegato alla parrocchia deve essere **approvato** (firmato con certificato HMAC-SHA256 legato al segreto della parrocchia) prima di poter sincronizzare le classi. Approvazioni e revoche si gestiscono dal "Centro approvazioni" e si propagano peer-to-peer agli altri dispositivi.
- **Supplenze** — La gestione delle supplenze è disponibile solo in modalità "Associato" quando la parrocchia usa la modalità Responsabile: un catechista può delegare la propria classe a un collega con codice QR temporaneo e registro supplenze dedicato.
