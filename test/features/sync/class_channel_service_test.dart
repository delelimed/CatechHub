...
void main() {
  ...
  group('ClassChannelService — titolo e chiavi', () {
    test('getOrCreateKey crea una chiave e concede il titolo', () async {
      expect(ClassChannelService.hasTitle(classCode), isFalse);

      final key = await ClassChannelService.getOrCreateKey(
        classId: classId,
        classUniqueCode: classCode,
        className: className,
      );
    ...
    test(
      'getOrCreateKey è idempotente (restituisce la stessa chiave)',
      () async {
        final k1 = await ClassChannelService.getOrCreateKey(
          classId: classId,
          classUniqueCode: classCode,
          className: className,
        );
        final k2 = await ClassChannelService.getOrCreateKey(
          classId: classId,
          classUniqueCode: classCode,
          className: className,
        );
    ...
    test('revokeTitle revoca il titolo locale', () async {
    ...
    test('storeKey importa una chiave ricevuta', () async {
      final imported = await ClassChannelService.storeKey(
        classId: classId,
        classUniqueCode: classCode,
        className: className,
  ...
  group('ClassChannelService — cifratura/decifratura', () {
    test('encryptRecords → decryptRecords è reversibile', () async {
      await ClassChannelService.getOrCreateKey(
        classId: classId,
        classUniqueCode: classCode,
        className: className,
      ...
      final blob = await ClassChannelService.encryptRecords(classCode, records);
    ...
    test('il blob NON contiene il testo in chiaro (nessuna leak)', () async {
      await ClassChannelService.getOrCreateKey(
        classId: classId,
        classUniqueCode: classCode,
        className: className,
      ...
      final blob = (await ClassChannelService.encryptRecords(
        classCode,
        records,
      ))!;
    ...
    test('decryptRecords senza titolo restituisce null (relay-only)', () async {
      await ClassChannelService.getOrCreateKey(
        classId: classId,
        classUniqueCode: classCode,
        className: className,
      ...
      ClassChannelService.revokeTitle(classId);
      expect(await ClassChannelService.decryptRecords(classCode, blob), isNull);
    });

    test('decryptRecords con chiave diversa restituisce null', () async {
      await ClassChannelService.getOrCreateKey(
        classId: classId,
        classUniqueCode: classCode,
        className: className,
      ...
      await ClassChannelService.rotateKey(
        classId: classId,
        classUniqueCode: classCode,
        className: className,
      );
    ...
    test('encryptRecords con records vuoti restituisce null', () async {
      await ClassChannelService.getOrCreateKey(
        classId: classId,
        classUniqueCode: classCode,
        className: className,
      );
      expect(await ClassChannelService.encryptRecords(classCode, []), isNull);
    });
  });

  group('ClassChannelService — relay dei blob cifrati', () {
    test('store/take round-trip conserva il blob senza titolo', () async {
      await ClassChannelService.getOrCreateKey(
        classId: classId,
        classUniqueCode: classCode,
        className: className,
    ...
    test(
      'il blob relayed è decifrabile dopo l\'acquisizione del titolo',
      () async {
        final blob = await ClassChannelService.encryptRecords(
          classCode,
          sampleRecords(),
        ...
        final realBlob = (await ClassChannelService.encryptRecords(
          classCode,
          sampleRecords(),
        ))!;
        ClassChannelService.storeRelayedCiphertext(classCode, realBlob);
        expect(
          await ClassChannelService.decryptRecords(classCode, realBlob),
          isNotNull,
        );
      },
    );
  ...
  group('ClassChannelService — QR handshake del titolo', () {
    test('crea un grant QR e lo importa con il PIN corretto', () async {
      final key = await ClassChannelService.getOrCreateKey(
        classId: classId,
        classUniqueCode: classCode,
        className: className,
      ...
      final chunks = await ClassChannelService.createKeyGrantChunks(
        grantMap,
        pin,
      );
      ...
      final imported = await ClassChannelService.importKeyGrant(assembled, pin);
    ...
    test('PIN errato non importa il titolo', () async {
      final key = await ClassChannelService.getOrCreateKey(
        classId: classId,
        classUniqueCode: classCode,
        className: className,
    ...
    test('grant con type errato viene rifiutato', () async {
      final payload = {
        'v': 1,
        'type': 'qualcos_altro',
        'keyBase64': ClassChannelService.generateKeyBase64(),
      ...
      expect(await ClassChannelService.importKeyGrant(assembled, pin), isNull);
    });
  });
}
